...
#2018年国际语音通信会议上提出的一种特征提取方法。
    def get_spectrogram_interspeech2018(self, X):
        ...
        return X_features
#该代码实现了一个音频分段的功能
# 输入参数包括音频文件路径 wavfile，采样率 sample_rate
# 分段长度 segment_length，重叠长度 overlap 和是否填充 padding。
def segment(wavfile,
            sample_rate = 16000,
            segment_length = 2,
            overlap = 1,
...
#这是一个语音分类任务的特征提取和数据预处理的函数
#wavfiles：音频文件路径的列表。
# labels：每个音频文件对应的标签。
# num_label：分类任务中的类别数。
# features_to_use：要提取的音频特征类型。
# sample_rate：音频文件的采样率。
# nmfcc：MFCC特征的数量。
# train_overlap：训练数据中每个窗口的重叠部分（单位为秒）。
# test_overlap：测试数据中每个窗口的重叠部分（单位为秒）。
# segment_length：音频文件被分割成的每个窗口的长度（单位为秒）。
# split_rate：用于将数据集分为训练集和测试集的比例。
# featuresFileName：保存特征的文件名。
# toSaveFeatures：是否保存特征。
# aug：数据增强方式，可以是"upsampling"（上采样）或者None（不进行数据增强）。
# padding：对于长度不足的音频文件进行填充的方式。
def process(wavfiles,
            ...
            segment_length=2,
            split_rate = 0.8,
            featuresFileName = 'features_lyh.pkl',
            toSaveFeatures = True,
            aug=None,
            padding=None):
    # Split datatset
    ...
    print("train_x:",numpy.shape(train_X))
    print("train_y:",numpy.shape(train_y))
    print("val_dict:",numpy.shape(val_dict))
    print("info:",info)

    return train_X, train_y, val_dict, info


def process_Babycry(datasets_path,
                    LABEL_DICT,
                    datadir='data/',
    ...
    if featuresFileName is None:
        featuresFileName = f'{datadir}/features_{features_to_use}.pkl'
    ...
    for wavfile in allfiles:
        label = str(os.path.basename(wavfile).split('_')[0])
...
if __name__ == '__main__':
    ...
    if args.batch:
        ...
        for i in range(args.batch):
            featuresFileName = f'{datadir[0]}/features_{features_to_use}_{i}.pkl'
            # print(featuresFileName)
            train_X, train_y, val_dict,info = process_Babycry(args.datasets_path, Babycry_LABEL,
                                                         datadir=args.datadir,
                                                         features_to_use='mfcc',
                                                         featuresFileName=featuresFileName,
                                                         sample_rate=16000,
