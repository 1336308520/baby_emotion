...
def parse_args():
    ...
    parser.add_argument('--featuresFileName',default='features/features_mfcc.pkl',type=str,help='features file name')
    # model
    parser.add_argument('-m', '--model',default='CNN_Area',type=str,help='specify models')
...
def train(kws):
    ...
    if kws['weight']:
        ...
        nums = np.array([count['awake'], count['diaper'], count['hug'], count['hungry']])
...
if __name__ == '__main__':
    ...
    from torch.optim.lr_scheduler import CosineAnnealingLR, ExponentialLR, StepLR, _LRScheduler
    ...
    filename = f'{kws["features_to_use"]}'
    if kws['featuresFileName'] is None:
        kws['featuresFileName'] = f'features/features_{filename}.pkl'
    ...
    if kws['loadFeatures'] and os.path.exists(kws['featuresFileName']):
        ...
        from feature_util_lyh import *
        train_X, train_y, val_dict, info = process_Babycry(datasets_path, Babycry_LABEL, **kws)

    train_data = data_loader.DataSet(train_X, train_y)
    train_loader = DataLoader(train_data, batch_size=kws['batch_size'], shuffle=True)

    target_names = ['awake','diaper','hug','hungry']
    ...
    # 定义标签名称
    labels = ['awake', 'diaper', 'hug', 'hungry']
    # 绘制混淆矩阵
    fig, ax = plt.subplots(figsize=(6, 6))
    im = ax.imshow(best_ma, interpolation='nearest', cmap=plt.cm.Blues)
    # 添加轴标签
    ax.set(xticks=np.arange(best_ma.shape[1]),
           yticks=np.arange(best_ma.shape[0]),
    ...
    # 在单元格中添加数值标签
    for i in range(best_ma.shape[0]):
        for j in range(best_ma.shape[1]):
            ax.text(j, i, format(best_ma[i, j], 'd'),
                    ha="center", va="center", color="white" if best_ma[i, j] > best_ma.max() / 2. else "black")
    # 添加标题
